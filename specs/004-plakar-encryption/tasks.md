...
- [~] T056 Final gate B. **Passing**: `make build`, `go vet ./src/...`, all `src/internal/app` tests, zero `replace` directives, no `contrib/` tree, integration resolved at `v0.1.0`, plugin/test deps absent from the build closure, `flake.nix` vendorHash regenerated and `nix build .#default` green (T046), and all three E2E round-trips (T049). **Outstanding**: `make lint` unrun (golangci-lint absent); `make test` exits non-zero only on the pre-existing macOS-only `tools/neo4jwatchdog` build failure. **Blocked on integration, not on this work:** PR #163 conflicts with `main` (108 commits of divergence, 10 conflicting files incl. `go.mod` and `main.go`), so GitHub cannot build the merge ref and **no CI can run on it** — the spec directory also collides with `main`'s own `004-backup-retention-policy`. That merge is a separate decision.
