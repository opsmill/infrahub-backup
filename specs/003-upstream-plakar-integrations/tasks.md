...
- [ ] T002 Align module versions in `go.mod`: `kloset` → `v1.1.0`, `integration-fs`/`integration-s3` to the matching line; then run `scripts/update-vendor-hash.sh` (never hand-edit `flake.nix`). **⚠️ COUPLING (found 2026-06-30): bumping beta.6→v1.1.0 breaks `src/internal/app/plakar_backup.go:182` — `snapshot.NewSource(ctx, 0, imp)` changed signature (the `0`/int arg is now typed). This code is in the importer path being deleted/reworked, so T002 MUST land together with T014 (delete StreamingImporter) + T017 (rework backup), not as a standalone green commit. Build the tool with `make build`, not `go build ./...` (the latter pulls in the linux-only `tools/neo4jwatchdog`, which is being removed in T014 anyway).**
...
> **🟡 DRAFT STATUS (2026-06-30):** A compile-verified, `go vet`-clean draft of the library packages exists at `contrib/integration-neo4j/` (module `github.com/PlakarKorp/integration-neo4j`, builds against kloset v1.1.0). **Validated against the live Infrahub's Neo4j Enterprise 2025.10.1**: the online `neo4j-admin database backup --compress=false --from --to-path <db>` produces a single `<db>-<ts>.backup` artifact (emitDir walks it correctly), and `database restore --from-path <dir> [--overwrite-destination] <db>` restores it — my command construction is confirmed correct. The **Community offline path is also validated** (standalone Neo4j Community 2025.10.1): `database dump` → single `<db>.dump`, `database load --overwrite-destination` round-trips with data intact (output path must be neo4j-user-writable). Also validated the Postgres path: `pg_dumpall --globals-only` + `pg_dump -Fc -Z0 prefect` → valid dump. **Full in-process connector path exercised end-to-end against live data** (Enterprise 2025.10.1, 18,553 nodes): a cross-compiled runner (our-binary model: neo4j importer + integration-fs + kloset, no plakar CLI) did **backup** = Import→kloset snapshot (66 MB artifact, committed to `fs://` repo) and **restore** = snapshot→Export→stage→`neo4j-admin restore` (RESTORE_OK). Surfaced + fixed a real exporter bug (pass the artifact **file** to `--from-path`, not the stage dir). **Still pending:** bringing the restored DB online (runner/Neo4j-ops: neo4j-admin config/home + neo4j-user ownership + Enterprise `CREATE DATABASE` catalog — the tool's lifecycle job, not the connector), testcontainers suite, SDK plugin entrypoints, edition/Bolt-count manifest. Upstream PR **not** opened (outward, pending green tests + review).
