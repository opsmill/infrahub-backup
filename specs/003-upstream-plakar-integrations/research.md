...
> **⚠️ Partly superseded (2026-08-07).** R-decisions about consuming the Neo4j integration "from the fork via `go.mod replace` until upstream merges/publishes the standalone mirror" no longer apply — there is no upstream merge to wait for. It now lives at `opsmill/plakar-integration-neo4j` and is consumed as a tagged external dependency. See [SUPERSEDED-BY-006.md](./SUPERSEDED-BY-006.md).
