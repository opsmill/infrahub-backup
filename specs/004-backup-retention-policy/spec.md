...
- **Partial permissions**: credentials allow upload but not list/delete on the bucket. On the `create` path and on `prune --force`, every leg is attempted: the local location is still pruned, and the run exits with a failure naming the location that could not be pruned — on `create`, the error text distinguishes backup success from retention failure. On an interactive `prune`, a leg that cannot be listed aborts the run before any deletion, so nothing is pruned at any location; re-running with `--force` prunes the legs that can be listed. *(Clarified 2026-08-04, after implementation: the original wording said only "the local location is still pruned", which holds for `create` and `prune --force` but not for an interactive `prune` — confirming a preview that is known to be incomplete would delete a set the operator was never shown, so that path refuses instead. Behavior is unchanged and tested; only this description was imprecise. See `contracts/cli.md` § Corrections after implementation.)*
...
- **FR-011**: Retention configuration MUST be expressible through the tool's existing configuration channels — command-line options and environment variables (prefix `INFRAHUB_`) — so unattended schedulers can supply it without wrapping scripts. A value supplied through either channel that cannot express a rule MUST abort the run rather than resolve to "no retention". *(Corrected 2026-08-04, after implementation: the original wording also required a configuration file. The tool has never read one — `src/internal/app/cli.go` sets up environment binding only — so that clause specified a mechanism that does not exist, and documenting it would have let an operator configure retention in YAML and get a silent no-op. Scope narrowed to the two channels the tool actually has; implementing config-file support was considered and deliberately not done. See `contracts/cli.md` § Corrections after implementation.)*
