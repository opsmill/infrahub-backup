...
`infrahub-ops` (also aliased as `infrahubops`) is a Go-based CLI tool for managing and maintaining Infrahub instances. It provides backup/restore capabilities, task management, and environment detection for Infrahub deployments.

## Common Development Commands

### Building and Running

- `make build` - Build the binary to `bin/infrahub-ops`
- `make build-all` - Cross-compile for Linux, Darwin, and Windows (amd64/arm64)
...
The codebase follows a command-pattern architecture using Cobra for CLI structure:

### Core Components

1. **main.go** - Entry point and CLI command structure
   - Defines root command and all subcommands (backup, environment, task-manager)
   - Uses Cobra for CLI framework and Viper for configuration
   - Commands: `backup create`, `backup restore`, `environment detect`, `environment list`, `task-manager clean`

2. **infrahub.go** - Core application logic
...
3. **backup.go** - Backup and restore operations
...
4. **taskmanager.go** - Task management operations
   - PostgreSQL database connection management
   - Stale task cleanup operations
   - Uses embedded Python scripts for task operations

5. **utils.go** - Utility functions
...
- **Embedded Scripts**: Python scripts are embedded using Go's embed package (scripts directory)
...
The tool assumes Infrahub is deployed using Docker Compose with these service names:

- `database` (Neo4j)
- `task-manager-db` (PostgreSQL)
- `infrahub-server`, `task-worker`, `task-manager`, `task-manager-background-svc`
- `cache`, `message-queue`
