...
The binary is self-contained and works offline: it uses your existing Docker or kubectl access and performs no network access beyond the deployment itself.
...
| Backup (opt-in) | A backup produced by the standard backup behavior (`--include-backup`) |
