...
`--dry-run`, `--force`, and `--s3` are per-invocation switches and are deliberately not configurable through the environment or a configuration file.
...
| A location failed to prune (other locations are still attempted) | non-zero |
