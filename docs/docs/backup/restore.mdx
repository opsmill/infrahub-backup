...
Everything else about the restore is unchanged: `--latest` only decides which archive is used. Metadata validation, checksum verification, version compatibility checks, and the container stop and start sequence all behave exactly as they do when you name an archive, and every other `restore` flag composes with `--latest`.
