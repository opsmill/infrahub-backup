...
Use Infrahub Backup to create and restore complete, verifiable recovery points for an Infrahub deployment. Run it as a CLI against Docker Compose, or as a Helm chart on Kubernetes, to capture the graph database, the task-manager database, and artifact storage as a single archive you can restore from later.
...
Infrahub Backup coordinates backup and restore across deployment targets. During a backup it stops the relevant services, captures each database with the correct credentials, and packages everything so a restore is deterministic. During a restore it brings services back in dependency order, reapplies the data, and records logs so you can confirm the operation succeeded. The CLI runs the provider-specific Docker or Kubernetes commands for you and sequences each step.
