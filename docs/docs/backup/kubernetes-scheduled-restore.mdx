...
| `restore.options.extraArgs` | Extra `restore` flags. The chart has no dedicated key for `--reset-deployment-id`. |
...
A restored instance inherits the deployment ID of the instance the backup came from, so without `--reset-deployment-id` the refreshed target reports the same identity as the source. The flag generates a new UUID after the database is restored and before the Infrahub containers start, so the application never observes the source deployment ID. See [Reset the deployment ID](./restore.mdx#reset-the-deployment-id).
...
- [ ] The target reports its own deployment ID, not the source's
