...
Before initiating a backup, check if there are active tasks that might be interrupted:
...
```bash
# Monitor task status
watch -n 5 'infrahubops taskmanager status'

# Once clear, proceed with backup
...
infrahubops backup restore infrahub_backups/infrahub_backup_20250929_143022.tar.gz --project=infrahub-staging --dry-run

# Review what would be restored
infrahubops backup info infrahub_backups/infrahub_backup_20250929_143022.tar.gz
```
...
- [CLI command reference](../reference/commands.mdx#backup-commands)